 ...
 * @return {*}
 */
void Common_IMU_GetEulerAngle(A_G_Struct_t *gyroAccel,
                              ...
                              float dt)
{
    ...
    NormQuat = Q_rsqrt(squa(gyroAccel->accel.x) +
                       squa(gyroAccel->accel.y) +
                       squa(gyroAccel->accel.z));

    Acc.x = gyroAccel->accel.x * NormQuat;
    Acc.y = gyroAccel->accel.y * NormQuat;
    Acc.z = gyroAccel->accel.z * NormQuat;
    // 向量差乘得出的值
    ...
    // 将Z轴角速度陀螺仪值 转换为Z角度/秒   Gyro_G陀螺仪初始化量程+-2000度每秒于1 / (65536 / 4000) = 0.03051756*2
    ...
    normAccz = gyroAccel->accel.x * vecxZ + gyroAccel->accel.y * vecyZ + gyroAccel->accel.z * veczZ; /*Z轴垂直方向上的加速度，此值涵盖了倾斜时在Z轴角速度的向量和，不是单纯重力感应得出的值*/
}
